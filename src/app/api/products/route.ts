...
export async function GET(req: NextRequest) {
  ...
  const conditions: string[] = ["category = 'น้ำมันเครื่อง'", "price1 > 0"]
  ...
  try {
    const [dataRes, countRes, brandsRes] = await Promise.all([
      pool.query(
        `SELECT bcode,pcode,descr,model,brand,price1,price5,qtyoh2,image_url
      ...
      pool.query(
        `SELECT DISTINCT brand FROM products WHERE category = 'น้ำมันเครื่อง' AND price1 > 0 AND brand IS NOT NULL AND brand != '' ORDER BY brand`
      ),
    ])
